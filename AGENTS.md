...
### 2026-05-05 对话 24：修复板块详情大面积加载失败

- 用户反馈板块页除“商业航天”外，大部分板块点击后详情加载失败。
- 已确认后端接口抽样中商业航天、半导体、国产芯片、北斗导航、军工、国防军工、先进封装等板块详情均能返回 200 和成分股，问题主要来自前端第一次点击时若详情请求被浏览器/隧道中断，就把右侧详情整体替换成失败提示。
- 已修改 `backend/static/index.html`：点击板块时先用左侧列表已有的资金流、涨跌、新闻统计和龙头字段构造基础详情，立即渲染右侧；后台再补全成分股和新闻时间线。
- 已把板块详情请求超时时间从 16 秒提升到 30 秒；请求失败时保留基础详情并显示温和提示，不再整块空白。
- 已修改 `backend/app/services/sector_service.py`：当成分股数据源暂不可用时，详情接口仍返回 `partial_without_constituents` 基础详情，包含资金流、涨跌和新闻归因。
- 已完成后端 `compileall app`、`preview_server.py` 语法检查、前端内联脚本语法检查；并用 FastAPI TestClient 抽样验证前 12 个热门板块详情均返回成功。已重启本地 `http://127.0.0.1:8000` 服务。

### 2026-05-05 对话 25：修复朋友端刷新新闻失败

- 用户反馈朋友那边点击“刷新新闻”显示刷新失败。
- 已定位前端 `refreshNewsFeed()` 调用 `/api/v1/quant/news/refresh` 时没有显式传入长超时，仍使用 `api()` 默认 12 秒；朋友端经 Cloudflare 隧道或公网访问时，多源新闻聚合容易被浏览器提前中断。
- 已修改 `backend/static/index.html`：新闻刷新请求超时提升到 90 秒，并使用 `noCache`；若请求失败，前端保留当前缓存新闻并提示“新闻刷新未完成，已保留当前缓存”，不再把页面状态清空。
- 已修改 `backend/app/services/news_service.py`：刷新新闻异常时自动回退到内存/归档缓存新闻，并在 `source_meta` 标记 `cache_fallback` 与 `refresh_error`。
- 已修改 `backend/app/main_simple.py`：`POST /api/v1/quant/news/refresh` 外层增加异常兜底，始终优先返回可用缓存新闻；返回体新增 `cache_fallback` 与 `error` 字段。
- 已完成后端 `compileall app`、`preview_server.py` 语法检查、前端内联脚本语法检查；TestClient 验证新闻刷新接口返回 200，本地样本刷新 8 个来源、267 条原始新闻、220 条去重新闻，合并归档后 1447 条。已重启本地 `http://127.0.0.1:8000` 服务。

### 2026-05-05 对话 26：修复板块热门股和新闻详情补全卡住

- 用户反馈板块详情中“龙头股/热门股”和“板块新闻时间线”加载不出来。
- 已确认截图中的右侧详情处于前端兜底状态：资金流基础数据已显示，但完整详情请求超时，导致热门股和新闻补全为空。
- 根因是 `get_sector_full_detail` 在每次点击板块时调用 `build_sector_news_map(sectors)`，会重算全市场约 200 个板块新闻归因并写入大型归档文件；这会拖慢详情接口，尤其在隧道访问时容易超时。
- 已新增 `build_single_sector_news(sector_name, sectors)`，详情页只计算当前板块相关新闻，不再重算全市场新闻归因。
- 已将 `data_fetcher.fetch_sector_detail` 的东方财富成分股请求超时从 10 秒降到 4 秒，避免外部源抖动拖死详情；成功后仍写入 `sector_detail_{code}.json` 缓存。
- 已完成后端 `compileall app`、`preview_server.py` 语法检查、前端内联脚本语法检查；TestClient 验证 `BK0629` 北斗导航详情 0.8 秒返回且有 10 个热门股，半导体、军工、先进封装等缓存板块 0.01 秒返回。已重启本地 `http://127.0.0.1:8000` 服务。

### 2026-05-05 对话 27：板块详情拆分股票层缓存与新闻层自动刷新

- 用户要求板块详情进入缓存；板块对应新新闻抓到后自动刷新加载进详情；上方数据属于股票数据，对应板块获取完后闭市期间不需要反复动。
- 已修改 `backend/app/services/sector_service.py`：新增板块股票层缓存策略，`sector_detail_{code}.json` 缓存成分股、热门股、龙头股和 `cached_at`；闭市期间直接使用缓存，盘中按短 TTL 刷新。
- 详情接口返回新增 `stock_cache` 字段，标明是否命中缓存、缓存时间、是否交易时段和缓存策略。
- 已保持新闻层独立动态：详情每次请求只重新计算当前板块相关新闻，并与 `sector_news_archive` 合并成 `news_timeline`，不触发股票层重抓。
- 已修改 `backend/static/index.html`：点击板块时即使已有前端缓存，也会轻量请求一次后端以更新新闻层；刷新新闻后若当前停留在板块页，会自动重新拉取当前板块详情，把新归因新闻补进右侧时间线。
- 已完成后端 `compileall app`、`preview_server.py` 语法检查、前端内联脚本语法检查；TestClient 验证闭市下 `BK0629` 第二次详情 0.006 秒缓存返回，半导体新闻时间线保留 44 条。已重启本地 `http://127.0.0.1:8000` 服务。

### 2026-05-05 对话 28：板块详情计算中显示正在加载

- 用户要求正在计算/加载时不要显示“暂无”，应显示“正在加载中”。
- 已修改 `backend/static/index.html`：板块详情 fallback 增加 `loading` 标记；在完整详情请求返回前，龙头股/热门股和板块新闻时间线显示“正在加载中...”。
- 只有接口明确返回完成且对应数组为空时，才显示“暂无成分股数据”或“暂无板块利好利空时间线”。
- 已完成前端内联脚本语法检查、`preview_server.py` 语法检查和后端 `compileall app`；已重启本地 `http://127.0.0.1:8000` 服务。
...
- 2026-05-04 用户反馈临时公网地址返回 502 Bad Gateway；已将临时隧道脚本和部署文档中的源站地址从 `localhost:8000` 改为更稳定的 `127.0.0.1:8000`，并在脚本中增加 `/api/v1/health` 本地健康检查，避免后端不可达时仍启动隧道。

### 2026-05-05 对话 25：首屏按需加载与交易时段轮询优化

- 用户反馈临时公网访问和数据加载等待过久，要求页面只加载当前可见板块；股票实时数据、新闻等需要后台捕获的数据继续捕获，实时行情只在交易日 9:00-15:00 左右捕获加载，其他页面点击后现场加载。
- 已改造 `backend/static/index.html` 的前端启动策略：`bootstrap()` 不再首屏并发拉取股票全集、板块、持仓、信号、风控、AI配置、Agent 等十几个接口；首屏先渲染框架，再轻量拉取健康状态、系统状态、熔断、缓存状态和少量新闻。
- 新增 `ensureSectionData()` 与多组页面级刷新函数：行情、板块、选股、信号、持仓、新闻、风控、数据中心、AI配置、Agent 工作台均改为进入对应页面后再按需加载。
- 首页只补充组合、持仓、风控和 AI 托管概要；行情页默认只拉前 300 条列表，搜索和股票详情仍按用户输入/点击即时请求。
- 后台刷新从每 15 秒全量 `refreshAll()` 改为：每 60 秒轻量刷新服务/熔断/缓存/少量新闻；交易时段内仅对当前打开的行情、信号、持仓页面做 15 秒刷新。
- 个股详情实时轮询已加交易时段判断，非交易时段不再每秒请求实时快照，避免无意义接口压力。
- 已完成 `backend/static/index.html` 内联脚本语法检查和 `preview_server.py` 编译检查。
